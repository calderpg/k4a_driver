...
int main(int argc, char** argv)
{
  ...
  const int32_t device_number = int64_to_int32(node->declare_parameter("device_number", 0));
  ...
  const int32_t subordinate_sync_delay_usec =
      int64_to_int32(node->declare_parameter("subordinate_sync_delay_usec", 160));
  ...
  if (color_resolution == "720p" || color_resolution == "720P")
  {
    ...
    RCLCPP_INFO(node->get_logger(), "Using color resolution K4A_COLOR_RESOLUTION_720P");
  }
  else if (color_resolution == "1080p" || color_resolution == "1080P")
  {
    config.color_resolution = K4A_COLOR_RESOLUTION_1080P;
    RCLCPP_INFO(node->get_logger(), "Using color resolution K4A_COLOR_RESOLUTION_1080P");
  }
  else if (color_resolution == "1440p" || color_resolution == "1440P")
  {
    config.color_resolution = K4A_COLOR_RESOLUTION_1440P;
    RCLCPP_INFO(node->get_logger(), "Using color resolution K4A_COLOR_RESOLUTION_1440P");
  }
  else if (color_resolution == "1536p" || color_resolution == "1536P")
  {
    config.color_resolution = K4A_COLOR_RESOLUTION_1536P;
    RCLCPP_INFO(node->get_logger(), "Using color resolution K4A_COLOR_RESOLUTION_1536P");
  }
  else if (color_resolution == "2160p" || color_resolution == "2160P")
  {
    config.color_resolution = K4A_COLOR_RESOLUTION_2160P;
    RCLCPP_INFO(node->get_logger(), "Using color resolution K4A_COLOR_RESOLUTION_2160P");
  }
  else if (color_resolution == "3072p" || color_resolution == "3072P")
  {
    config.color_resolution = K4A_COLOR_RESOLUTION_3072P;
    RCLCPP_INFO(node->get_logger(), "Using color resolution K4A_COLOR_RESOLUTION_3072P");
  }
  else
  {
  ...
  if (enable_wide_depth_fov)
  {
    if (enable_depth_binning)
    {
      ...
      RCLCPP_INFO(node->get_logger(), "Using depth mode K4A_DEPTH_MODE_WFOV_2X2BINNED");
    }
    else
    {
      config.depth_mode = K4A_DEPTH_MODE_WFOV_UNBINNED;
      RCLCPP_INFO(node->get_logger(), "Using depth mode K4A_DEPTH_MODE_WFOV_UNBINNED");
    }
  }
  else
  {
    if (enable_depth_binning)
    {
      ...
      RCLCPP_INFO(node->get_logger(), "Using depth mode K4A_DEPTH_MODE_NFOV_2X2BINNED");
    }
    else
    {
      config.depth_mode = K4A_DEPTH_MODE_NFOV_UNBINNED;
      RCLCPP_INFO(node->get_logger(), "Using depth mode K4A_DEPTH_MODE_NFOV_UNBINNED");
    }
  }
  ...
  if (sync_mode == "master")
  {
    ...
    RCLCPP_INFO(node->get_logger(), "Using sync mode K4A_WIRED_SYNC_MODE_MASTER");
  }
  else if (sync_mode == "subordinate")
  {
    config.wired_sync_mode = K4A_WIRED_SYNC_MODE_SUBORDINATE;
    RCLCPP_INFO(node->get_logger(), "Using sync mode K4A_WIRED_SYNC_MODE_SUBORDINATE");
  ...
  else if (sync_mode == "standalone")
  {
    ...
    RCLCPP_INFO(node->get_logger(), "Using sync mode K4A_WIRED_SYNC_MODE_STANDALONE");
  }
  else
  {
    throw std::runtime_error(
  ...
  RCLCPP_INFO(node->get_logger(), "Using device_number [%i]", device_number);
  RCLCPP_INFO(node->get_logger(), "Using serial_number [%s]", serial_number.c_str());
  if (serial_number != "")
  {
